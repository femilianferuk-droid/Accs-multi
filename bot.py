...
Подписки, админ-панель по /admin
"""

import asyncio, logging, re, sys, uuid
...
from sqlalchemy import Column, Integer, BigInteger, String, Text, Numeric, Boolean, DateTime, ForeignKey, create_engine, text, func
...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://postgres:password@localhost:5432/vest_multi").replace("+asyncpg", "").replace("+psycopg", "")
...
TARIFFS = {
    "free": {"name": "Бесплатный", "max_bots": 1, "payments": [], "price": 0},
    "pro": {"name": "PRO", "max_bots": 5, "payments": ["crypto", "yoomoney", "rollypay"], "price": 50},
    "pro_max": {"name": "PRO MAX", "max_bots": 20, "payments": ["crypto", "yoomoney", "rollypay"], "price": 150}
}
...
def init_db():
    ...
    with engine.connect() as conn:
        for col, col_type, table in [
            ("yoomoney_token", "VARCHAR(255)", "shops"),
    ...
    s = SessionLocal()
    try:
        if not s.query(AdminSettings).first():
            s.add(AdminSettings())
            s.commit()
    finally:
        s.close()
...
def kb_main():
    ...
    b.row(KeyboardButton(text="🤖 Создать бота"))
    b.row(KeyboardButton(text="📋 Мои боты"), KeyboardButton(text="👤 Профиль"))
    b.row(KeyboardButton(text="🆘 Поддержка"), KeyboardButton(text="📖 Инструкции"))
    return b.as_markup(resize_keyboard=True)

def kb_profile():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="⭐ Купить подписку", callback_data="buy_sub"))
    b.row(InlineKeyboardButton(text="🔙 Назад в главное меню", callback_data="main_menu"))
    return b.as_markup()

def kb_admin():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="📊 Статистика", callback_data="admin_stats"))
    b.row(InlineKeyboardButton(text="📢 Рассылка", callback_data="admin_brd_sel"))
    b.row(InlineKeyboardButton(text="👤 Выдать подписку", callback_data="admin_give_sub"))
    b.row(InlineKeyboardButton(text="💳 Платёжные реквизиты", callback_data="admin_pay_settings"))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
    return b.as_markup()

def kb_shop(sid):
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="📂 Управление категориями", callback_data=f"cats_{sid}"))
    b.row(InlineKeyboardButton(text="📦 Управление товарами", callback_data=f"prods_{sid}"))
    b.row(InlineKeyboardButton(text="💳 Платёжные реквизиты", callback_data=f"pay_{sid}"))
    b.row(InlineKeyboardButton(text="📝 Приветственное сообщение", callback_data=f"wel_{sid}"))
    b.row(InlineKeyboardButton(text="🎧 Изменить юзернейм поддержки", callback_data=f"sup_{sid}"))
    b.row(InlineKeyboardButton(text="👤 Выдать баланс пользователю", callback_data=f"bal_{sid}"))
    b.row(InlineKeyboardButton(text="📊 Статистика", callback_data=f"st_{sid}"))
    b.row(InlineKeyboardButton(text="📢 Рассылка", callback_data=f"brd_{sid}"))
    b.row(InlineKeyboardButton(text="🗑 Удалить бота", callback_data=f"del_{sid}"))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="my_bots"))
    return b.as_markup()

def kb_child():
    b = ReplyKeyboardBuilder()
    b.row(KeyboardButton(text="🛍 Купить аккаунт"))
    b.row(KeyboardButton(text="👤 Профиль"), KeyboardButton(text="📦 Мои покупки"))
    b.row(KeyboardButton(text="🆘 Поддержка"))
    return b.as_markup(resize_keyboard=True)

def kb_subs():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="⭐ PRO - 50₽", callback_data="sub_pro"))
    b.row(InlineKeyboardButton(text="👑 PRO MAX - 150₽", callback_data="sub_pro_max"))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="profile"))
    return b.as_markup()

def kb_sub_pay(key):
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="🪙 Crypto Bot", callback_data=f"psub_crypto_{key}"))
    b.row(InlineKeyboardButton(text="💳 ЮMoney", callback_data=f"psub_ym_{key}"))
    b.row(InlineKeyboardButton(text="💳 СБП", callback_data=f"psub_rp_{key}"))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="buy_sub"))
    return b.as_markup()

def kb_admin_pay():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="🪙 Crypto Bot Token", callback_data="aset_crypto"))
    b.row(InlineKeyboardButton(text="💳 ЮMoney", callback_data="aset_ym"))
    b.row(InlineKeyboardButton(text="💳 СБП (RollyPay)", callback_data="aset_rp"))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
    return b.as_markup()
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get("https://api.coingecko.com/api/v3/simple/price?ids=tether&vs_currencies=rub") as r:
                return Decimal(str((await r.json())["tether"]["rub"]))
    except:
        return Decimal("90")
...
class CryptoBotAPI:
    ...
    async def create_invoice(token, amount_rub, description):
        try:
            ...
            data = {
                ...
                "hidden_message": f"Сумма в RUB: {amount_rub} ₽"
            }
            async with aiohttp.ClientSession() as s:
                async with s.post(f"{CRYPTO_BOT_API}/createInvoice", json=data, headers=headers) as r:
                    res = await r.json()
                    if res.get("ok"):
                        return {
                            **res["result"],
                    ...
                    logger.error(f"Crypto Bot error: {res}")
                    return None
    ...
    async def check_payment(token, invoice_id):
        try:
            ...
            async with aiohttp.ClientSession() as s:
                async with s.post(f"{CRYPTO_BOT_API}/getInvoice", json=data, headers=headers) as r:
                    res = await r.json()
                    if res.get("ok"):
                        return res["result"]
                    return None
...
class RollyPayAPI:
    @staticmethod
    async def create_payment(api_key, terminal_id, amount, order_id, description):
        try:
            headers = {
            ...
            async with aiohttp.ClientSession() as s:
                async with s.post(f"{ROLLYPAY_API}/payments", json=data, headers=headers) as r:
                    if r.status == 200:
                        return await r.json()
                    logger.error(f"RollyPay error: {await r.text()}")
                    return None
        except Exception as e:
    ...
    async def check_payment(api_key, payment_id):
        try:
            ...
            async with aiohttp.ClientSession() as s:
                async with s.get(f"{ROLLYPAY_API}/payments/{payment_id}", headers=headers) as r:
                    if r.status == 200:
                        return await r.json()
                    return None
        except Exception as e:
...
class TelethonManager:
    ...
    async def get_latest_code(self, session_string):
        ...
        try:
            ...
            dialogs_sorted = sorted(dialogs, key=lambda d: d.date if d.date else datetime.min, reverse=True)
            for dialog in dialogs_sorted:
                if dialog.is_user:
                    if hasattr(dialog.entity, 'username') and dialog.entity.username in ['Telegram', 'telegram']:
                        continue
                ...
                for message in messages:
                    if message.text:
                        ...
                        if codes:
                            logger.info(f"Найден код {codes[0]} в чате {dialog.name} от {message.date}")
                            return codes[0]
            logger.warning("Код не найден ни в одном чате")
            return None
        except Exception as e:
...
def format_tariff_name(tariff_key):
    return TARIFFS.get(tariff_key, {}).get("name", "Бесплатный")

async def complete_purchase_and_notify(callback, session, purchase):
    """Завершает покупку и отправляет уведомление"""
    ...
    try:
        ...
        async def child_start(message: Message):
            ...
            try:
                ...
                welcome_text = shop_data.welcome_message or "👋 Добро пожаловать в наш магазин!"
        ...
        async def child_category_products(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    await callback.message.edit_text("📦 Выберите товар:", reply_markup=b.as_markup())
                finally:
                    session.close()
            except Exception as e:
        ...
        async def child_product_detail(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    shop_data = session.query(Shop).filter(Shop.id == shop.id).first()
                    owner = session.query(User).filter(User.telegram_id == shop_data.owner_id).first()
                    if not product:
                        await callback.message.answer("❌ Товар не найден")
                        return
                    text = f"📛 {product.name}\n📝 {product.description or 'Нет описания'}\n💵 Цена: {product.price} ₽\n\nВыберите способ оплаты:"
                    b = InlineKeyboardBuilder()
                    b.row(InlineKeyboardButton(text="💰 Баланс", callback_data=f"pay_balance_{product.id}"))
                    owner_tariff = owner.tariff if owner else "free"
                    available = TARIFFS.get(owner_tariff, TARIFFS["free"])["payments"]
                    if "crypto" in available and shop_data and shop_data.crypto_bot_token:
                        b.row(InlineKeyboardButton(text="🪙 Crypto Bot", callback_data=f"pay_crypto_{product.id}"))
                    if "yoomoney" in available and shop_data and shop_data.yoomoney_wallet:
                        b.row(InlineKeyboardButton(text="💳 ЮMoney", callback_data=f"pay_yoomoney_{product.id}"))
                    if "rollypay" in available and shop_data and shop_data.rollypay_terminal_id:
                        b.row(InlineKeyboardButton(text="💳 СБП", callback_data=f"pay_rollypay_{product.id}"))
                    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"child_cat_{product.category_id}"))
        ...
        async def pay_crypto(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    b.row(InlineKeyboardButton(text="🔄 Проверить оплату", callback_data=f"check_crypto_{purchase.id}"))
                    await callback.message.answer(
                        f"🪙 Счет создан!\n\n📛 Товар: {product.name}\n💵 Сумма: {product.price} ₽\n💎 Сумма в USDT: {invoice['amount_usdt']} USDT\n📊 Курс: 1 USDT = {invoice['rate']} ₽\n\nНажмите кнопку для оплаты:",
                        reply_markup=b.as_markup()
                    )
                finally:
                    session.close()
            ...
            try:
                ...
                try:
                    ...
                    if payment and payment["status"] == "paid":
                        await complete_purchase_and_notify(callback, session, purchase)
                    else:
        ...
        async def pay_yoomoney(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    b.row(InlineKeyboardButton(text="🔄 Проверить оплату", callback_data=f"check_yoomoney_{purchase.id}"))
                    await callback.message.answer(
        ...
        @child_router.callback_query(F.data.startswith("pay_rollypay_"))
        async def pay_rollypay(callback: CallbackQuery):
            try:
                ...
                try:
                    product = session.query(Product).filter(Product.id == product_id).first()
                    shop_data = session.query(Shop).filter(Shop.id == shop.id).first()
                    if not product or not shop_data or not shop_data.rollypay_terminal_id:
                        await callback.message.answer("❌ Оплата СБП не настроена")
                        return
                    ...
                    session.refresh(purchase)
                    b = InlineKeyboardBuilder()
                    b.row(InlineKeyboardButton(text="💳 Оплатить СБП", url=payment["pay_url"]))
                    b.row(InlineKeyboardButton(text="🔄 Проверить оплату", callback_data=f"check_rollypay_{purchase.id}"))
                    await callback.message.answer(
        ...
        @child_router.callback_query(F.data.startswith("check_rollypay_"))
            ...
            try:
                ...
                try:
                    ...
                    if not purchase or purchase.user_id != callback.from_user.id:
                        ...
                        return
                    if purchase.status != "completed":
                        await callback.message.answer("❌ Покупка не оплачена")
                        return
                    product = session.query(Product).filter(Product.id == purchase.product_id).first()
                    if not product or not product.session_string:
                        await callback.message.answer("❌ Невозможно получить код")
                        return
                    await callback.message.answer("🔍 Ищем свежий код подтверждения...")
                    ...
                    if code:
                        ...
                        await callback.message.answer("❌ Не удалось найти код. Попробуйте позже или обратитесь в поддержку.")
                finally:
                    session.close()
            except Exception as e:
...
async def cmd_start(message: Message):
    ...
    await message.answer("🤖 Vest Multi\n\nДобро пожаловать в главное меню!", reply_markup=kb_main())
...
async def back_to_main(callback: CallbackQuery):
    await callback.message.answer("🤖 Vest Multi\n\nГлавное меню:", reply_markup=kb_main())
...
async def show_profile(message: Message):
    ...
    try:
        ...
        if not user:
            ...
            return
        tariff_name = format_tariff_name(user.tariff)
...
async def create_bot_start(message: Message, state: FSMContext):
    ...
    try:
        user = session.query(User).filter(User.telegram_id == message.from_user.id).first()
        max_bots = TARIFFS.get(user.tariff if user else "free", TARIFFS["free"])["max_bots"]
        bot_count = session.query(Shop).filter(Shop.owner_id == message.from_user.id).count()
        if bot_count >= max_bots:
            await message.answer(f"❌ Достигнут лимит ботов ({max_bots}) для вашего тарифа!\n⭐ Повысьте тариф в профиле")
            return
    finally:
        session.close()
...
async def show_my_bots(message: Message):
    ...
    try:
        ...
        if not shops:
            await message.answer("📋 У вас пока нет созданных ботов", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu")]]))
            return
        b = InlineKeyboardBuilder()
        for shop in shops:
            status_emoji = "🟢" if shop.status == "active" else "🔴"
            b.row(InlineKeyboardButton(text=f"🤖 {shop.bot_name} (@{shop.bot_username}) — {status_emoji}", callback_data=f"shop_menu_{shop.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
        await message.answer("📋 Ваши боты:", reply_markup=b.as_markup())
    finally:
        session.close()
...
async def buy_subscription(callback: CallbackQuery):
    await callback.message.edit_text("⭐ Выберите тариф:", reply_markup=kb_subs())
...
async def pay_sub_crypto(callback: CallbackQuery):
    ...
    if not admin_settings or not admin_settings.crypto_bot_token:
        await callback.message.answer("❌ Администратор еще не настроил Crypto Bot")
        return
    invoice = await CryptoBotAPI.create_invoice(
...
async def pay_sub_yoomoney(callback: CallbackQuery):
    ...
    if not admin_settings or not admin_settings.yoomoney_wallet:
        await callback.message.answer("❌ Администратор еще не настроил ЮMoney")
        return
    label = str(uuid.uuid4())
    payment_url = YooMoneyAPI.create_payment(
...
async def pay_sub_rollypay(callback: CallbackQuery):
    ...
    tariff = TARIFFS[tariff_key]
    admin_settings = get_admin_settings()
    if not admin_settings or not admin_settings.rollypay_terminal_id:
        await callback.message.answer("❌ Администратор еще не настроил СБП")
        return
    order_id = str(uuid.uuid4())
    payment = await RollyPayAPI.create_payment(
...
async def admin_stats(callback: CallbackQuery):
    ...
    try:
        ...
        purchases_count = session.query(Purchase).filter(Purchase.status == "completed").count()
        total_revenue = session.query(func.sum(Purchase.price)).filter(Purchase.status == "completed").scalar() or 0
        text = f"📊 Статистика\n\n👥 Пользователей: {users_count}\n🤖 Ботов: {shops_count}\n🛍 Продаж: {purchases_count}\n💰 Выручка: {total_revenue} ₽"
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu")]]))
    finally:
        session.close()
...
async def admin_give_sub_user(message: Message, state: FSMContext):
    try:
        ...
        for key, tariff in TARIFFS.items():
            if key != "free":
                b.row(InlineKeyboardButton(text=f"{tariff['name']} - {tariff['price']}₽", callback_data=f"admin_set_sub_{key}"))
        await message.answer("⭐ Выберите тариф:", reply_markup=b.as_markup())
        await state.set_state(AdminGiveSubStates.waiting_tariff)
    except ValueError:
        await message.answer("❌ Неверный ID")

@router.callback_query(AdminGiveSubStates.waiting_tariff, F.data.startswith("admin_set_sub_"))
async def admin_give_sub_finish(callback: CallbackQuery, state: FSMContext):
    tariff_key = callback.data.split("_")[3]
    data = await state.get_data()
    session = SessionLocal()
    try:
        user = get_or_create_user_sync(data["target_user_id"], "")
        user.tariff = tariff_key
        user.tariff_expires = None
        session.commit()
        await callback.message.answer(f"✅ Подписка {TARIFFS[tariff_key]['name']} выдана пользователю {data['target_user_id']}")
    finally:
        session.close()
        await state.clear()
...
async def admin_payment_settings(callback: CallbackQuery):
    ...
    admin_settings = get_admin_settings()
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text=f"🪙 Crypto Bot Token {'✅' if admin_settings and admin_settings.crypto_bot_token else '❌'}", callback_data="aset_crypto"))
    b.row(InlineKeyboardButton(text=f"💳 ЮMoney {'✅' if admin_settings and admin_settings.yoomoney_wallet else '❌'}", callback_data="aset_ym"))
    b.row(InlineKeyboardButton(text=f"💳 СБП (RollyPay) {'✅' if admin_settings and admin_settings.rollypay_terminal_id else '❌'}", callback_data="aset_rp"))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
    await callback.message.edit_text("💳 Платёжные реквизиты (для оплаты подписок)", reply_markup=b.as_markup())

@router.callback_query(F.data == "aset_crypto")
async def admin_set_crypto(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminPaymentStates.waiting_crypto_token)

@router.message(AdminPaymentStates.waiting_crypto_token)
async def admin_save_crypto(message: Message, state: FSMContext):
    session = SessionLocal()
    try:
...
@router.callback_query(F.data == "aset_ym")
async def admin_set_yoomoney(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        return
    await callback.message.edit_text("💳 Введите данные ЮMoney в формате:\nномер_кошелька:токен_доступа")
    await state.set_state(AdminPaymentStates.waiting_yoomoney_wallet)

@router.message(AdminPaymentStates.waiting_yoomoney_wallet)
async def admin_save_yoomoney(message: Message, state: FSMContext):
    session = SessionLocal()
    try:
        parts = message.text.strip().split(":")
        admin_settings = session.query(AdminSettings).first()
        admin_settings.yoomoney_wallet = parts[0]
        if len(parts) > 1:
            admin_settings.yoomoney_token = parts[1]
        session.commit()
...
@router.callback_query(F.data == "aset_rp")
async def admin_set_rollypay_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        return
    await callback.message.edit_text("💳 Введите Terminal ID:")
    await state.set_state(AdminPaymentStates.waiting_rollypay_terminal)

@router.message(AdminPaymentStates.waiting_rollypay_terminal)
async def admin_set_rollypay_terminal(message: Message, state: FSMContext):
    await state.update_data(rp_terminal=message.text.strip())
    await message.answer("🔑 Введите API Key:")
    await state.set_state(AdminPaymentStates.waiting_rollypay_api_key)

@router.message(AdminPaymentStates.waiting_rollypay_api_key)
async def admin_set_rollypay_api_key(message: Message, state: FSMContext):
    await state.update_data(rp_api_key=message.text.strip())
    await message.answer("🔐 Введите Signing Secret:")
    await state.set_state(AdminPaymentStates.waiting_rollypay_secret)

@router.message(AdminPaymentStates.waiting_rollypay_secret)
async def admin_save_rollypay(message: Message, state: FSMContext):
    session = SessionLocal()
    try:
        data = await state.get_data()
        admin_settings = session.query(AdminSettings).first()
        admin_settings.rollypay_terminal_id = data["rp_terminal"]
        admin_settings.rollypay_api_key = data["rp_api_key"]
        admin_settings.rollypay_signing_secret = message.text.strip()
        session.commit()
...
async def admin_broadcast_select(callback: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder()
        for shop in shops:
            b.row(InlineKeyboardButton(text=f"🤖 {shop.bot_name}", callback_data=f"admin_broadcast_{shop.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
        await callback.message.edit_text("📢 Выберите бота для рассылки:", reply_markup=b.as_markup())
    finally:
        session.close()
...
async def broadcast_execute(message: Message, state: FSMContext):
    ...
    try:
        ...
        if shop.id in child_bots:
            ...
            purchases = session.query(Purchase.user_id).filter(Purchase.shop_id == shop_id).distinct().all()
            sent_count = 0
            for (user_id,) in purchases:
                try:
                    await child_bot.send_message(user_id, message.text)
                    sent_count += 1
                    await asyncio.sleep(0.05)
                except Exception:
                    continue
            await message.answer(f"✅ Рассылка завершена! Отправлено: {sent_count}")
        else:
...
async def shop_menu(callback: CallbackQuery):
    ...
    session = SessionLocal()
    try:
        shop = session.query(Shop).filter(Shop.id == shop_id, Shop.owner_id == callback.from_user.id).first()
        if not shop:
            await callback.message.answer("❌ Бот не найден")
            return
        text = f"🤖 {shop.bot_name} (@{shop.bot_username})\n🟢 Статус: {shop.status}\n🎧 Поддержка: @{shop.support_username}"
        await callback.message.edit_text(text, reply_markup=kb_shop(shop_id))
    finally:
        session.close()
...
async def manage_categories(callback: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="➕ Добавить категорию", callback_data=f"add_category_{shop_id}"))
        for cat in categories:
            b.row(InlineKeyboardButton(text=f"📁 {cat.name}", callback_data=f"edit_category_{cat.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"shop_menu_{shop_id}"))
        await callback.message.edit_text("📂 Управление категориями:", reply_markup=b.as_markup())
    finally:
        session.close()
...
async def edit_category_menu(callback: CallbackQuery):
    ...
    try:
        ...
        if not category:
            ...
            return
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="🗑 Удалить", callback_data=f"delete_category_{cat_id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"cats_{category.shop_id}"))
        await callback.message.edit_text(f"📁 Категория: {category.name}", reply_markup=b.as_markup())
    finally:
        session.close()
...
async def delete_category(callback: CallbackQuery):
    ...
    try:
        ...
        session.commit()
        await callback.message.edit_text("✅ Категория удалена", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"cats_{shop_id}")]]))
    finally:
        session.close()
...
async def manage_products(callback: CallbackQuery):
    ...
    try:
        categories = session.query(Category).filter(Category.shop_id == shop_id).all()
        if not categories:
            await callback.message.edit_text("📂 Сначала добавьте категорию!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"shop_menu_{shop_id}")]]))
            return
        b = InlineKeyboardBuilder()
        for cat in categories:
            b.row(InlineKeyboardButton(text=f"📁 {cat.name}", callback_data=f"show_products_{cat.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"shop_menu_{shop_id}"))
        await callback.message.edit_text("📦 Выберите категорию:", reply_markup=b.as_markup())
    finally:
        session.close()
...
async def show_products(callback: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="➕ Добавить товар", callback_data=f"add_product_{cat_id}"))
        for product in products:
            status = "🟢" if product.status == "available" else "🔴"
            b.row(InlineKeyboardButton(text=f"{status} {product.name} — {product.price} ₽", callback_data=f"product_menu_{product.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"prods_{category.shop_id}"))
        await callback.message.edit_text(f"📦 Товары в категории {category.name}:", reply_markup=b.as_markup())
    finally:
        session.close()
...
async def add_product_phone(message: Message, state: FSMContext):
    try:
        ...
        if not client:
            ...
            return
        await state.update_data(phone=phone, client_session=client.session.save(), phone_code_hash=phone_code_hash)
...
async def add_product_code(message: Message, state: FSMContext):
    try:
        ...
        client = TelegramClient(StringSession(data["client_session"]), telethon_manager.api_id, telethon_manager.api_hash)
        await client.connect()
        session_string, password_2fa, has_2fa = await telethon_manager.sign_in(client, data["phone"], code, data["phone_code_hash"])
...
async def add_product_2fa(message: Message, state: FSMContext):
    try:
        ...
        client = TelegramClient(StringSession(data["client_session"]), telethon_manager.api_id, telethon_manager.api_hash)
        await client.connect()
        session_string, saved_2fa, has_2fa = await telethon_manager.sign_in(client, data["phone"], None, data["phone_code_hash"], password_2fa)
        if session_string is None:
...
async def save_product(message, state, session_string, password_2fa, has_2fa):
    session = SessionLocal()
    try:
        data = await state.get_data()
...
async def product_menu(callback: CallbackQuery):
    ...
    try:
        ...
        if not product:
            ...
            return
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="🗑 Удалить", callback_data=f"delete_product_{product.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"show_products_{product.category_id}"))
        text = f"📛 {product.name}\n📝 {product.description or 'Нет описания'}\n💵 {product.price} ₽\n📱 {product.phone_number}\n🟢 {product.status}"
        await callback.message.edit_text(text, reply_markup=b.as_markup())
    finally:
        session.close()
...
async def delete_product(callback: CallbackQuery):
    ...
    try:
        ...
        session.commit()
        await callback.message.edit_text("✅ Товар удалён", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"show_products_{cat_id}")]]))
    finally:
        session.close()
...
async def delete_bot_confirm(callback: CallbackQuery):
    shop_id = int(callback.data.split("_")[1])
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="✅ Да, удалить", callback_data=f"confirm_delete_{shop_id}"))
    b.row(InlineKeyboardButton(text="❌ Нет, отмена", callback_data=f"shop_menu_{shop_id}"))
    await callback.message.edit_text("⚠️ Вы уверены, что хотите удалить бота?", reply_markup=b.as_markup())
...
async def delete_bot_execute(callback: CallbackQuery):
    ...
    try:
        ...
        session.delete(shop)
        session.commit()
        await callback.message.edit_text("🗑 Бот удалён", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 В главное меню", callback_data="main_menu")]]))
    finally:
        session.close()
...
async def payment_settings(callback: CallbackQuery):
    ...
    try:
        shop = session.query(Shop).filter(Shop.id == shop_id).first()
        user = session.query(User).filter(User.telegram_id == shop.owner_id).first()
        available = TARIFFS.get(user.tariff if user else "free", TARIFFS["free"])["payments"]
        b = InlineKeyboardBuilder()
        if "crypto" in available:
            b.row(InlineKeyboardButton(text=f"🪙 Crypto Bot Token {'✅' if shop.crypto_bot_token else '❌'}", callback_data=f"set_crypto_{shop_id}"))
        if "yoomoney" in available:
            b.row(InlineKeyboardButton(text=f"💳 ЮMoney кошелёк {'✅' if shop.yoomoney_wallet else '❌'}", callback_data=f"set_yoomoney_{shop_id}"))
        if "rollypay" in available:
            b.row(InlineKeyboardButton(text=f"💳 СБП (RollyPay) {'✅' if shop.rollypay_terminal_id else '❌'}", callback_data=f"set_rollypay_{shop_id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"shop_menu_{shop_id}"))
        await callback.message.edit_text("💳 Платёжные реквизиты", reply_markup=b.as_markup())
    finally:
        session.close()
...
async def set_rollypay_start(callback: CallbackQuery, state: FSMContext):
    shop_id = int(callback.data.split("_")[2])
    await state.update_data(shop_id=shop_id)
    await callback.message.edit_text("💳 Введите Terminal ID:")
    await state.set_state(PaymentSettingsStates.waiting_rollypay_terminal)

@router.message(PaymentSettingsStates.waiting_rollypay_terminal)
async def set_rollypay_terminal(message: Message, state: FSMContext):
    await state.update_data(rollypay_terminal=message.text.strip())
    await message.answer("🔑 Введите API Key:")
    await state.set_state(PaymentSettingsStates.waiting_rollypay_api_key)

@router.message(PaymentSettingsStates.waiting_rollypay_api_key)
async def set_rollypay_api_key(message: Message, state: FSMContext):
    await state.update_data(rollypay_api_key=message.text.strip())
    await message.answer("🔐 Введите Signing Secret:")
    await state.set_state(PaymentSettingsStates.waiting_rollypay_secret)

@router.message(PaymentSettingsStates.waiting_rollypay_secret)
async def set_rollypay_save(message: Message, state: FSMContext):
    session = SessionLocal()
    try:
        data = await state.get_data()
        shop_id = data["shop_id"]
        session.query(Shop).filter(Shop.id == shop_id).update({
            "rollypay_terminal_id": data["rollypay_terminal"],
            "rollypay_api_key": data["rollypay_api_key"],
            "rollypay_signing_secret": message.text.strip()
        })
        session.commit()
        await message.answer("✅ RollyPay сохранён!")
    finally:
...
async def shop_stats(callback: CallbackQuery):
    ...
    session = SessionLocal()
    try:
        total_sales = session.query(Purchase).filter(Purchase.shop_id == shop_id, Purchase.status == "completed").count()
        total_revenue = session.query(func.sum(Purchase.price)).filter(Purchase.shop_id == shop_id, Purchase.status == "completed").scalar() or 0
        total_products = session.query(Product).filter(Product.shop_id == shop_id).count()
        text = f"📊 Статистика бота\n\n🛍 Продаж: {total_sales}\n💰 Выручка: {total_revenue} ₽\n📦 Товаров: {total_products}"
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"shop_menu_{shop_id}")]]))
    finally:
        session.close()
...
@router.message(BroadcastStates.waiting_message)
async def broadcast_execute_shop(message: Message, state: FSMContext):
    data = await state.get_data()
    shop_id = data.get("broadcast_shop_id")
    session = SessionLocal()
    try:
