...
class RollyPayAPI:
    ...
    async def create_payment(
        ...
        try:
            ...
            logger.info(
                f"RollyPay request: amount={data['amount']}, "
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            f"RollyPay check: "
                            f"{data.get('status')} for {payment_id}"
                        )
                        return data
                    logger.error(
                ...
                try:
                    ...
                    for product in products:
                        builder.row(InlineKeyboardButton(
                            ...
                            callback_data=f"cp_{product.id}"
                        ))
        ...
        async def child_product_detail(
            ...
            try:
                ...
                try:
                    ...
                    builder.row(InlineKeyboardButton(
                        text="💰 Баланс",
                        callback_data=f"pb_{product.id}"
                    ))
                    builder.row(InlineKeyboardButton(
                        text="🪙 Crypto Bot",
                        callback_data=f"pc_{product.id}"
                    ))
                    builder.row(InlineKeyboardButton(
                        text="💳 ЮMoney",
                        callback_data=f"py_{product.id}"
                    ))
                    builder.row(InlineKeyboardButton(
                        text="💳 СБП",
                        callback_data=f"pr_{product.id}"
                    ))
                    builder.row(InlineKeyboardButton(
                        text="🔙 Назад",
                        callback_data=(
                ...
                try:
                    ...
                    if not payment:
                        await callback.message.answer(
                            ...
                            f"Terminal ID: "
                            f"{shop_data.rollypay_terminal_id[:10]}"
                            f"...\n"
                            f"API Key: "
                            f"{shop_data.rollypay_api_key[:10]}"
                            f"...\n"
                            "Signing Secret: ***\n\n"
                            "Убедитесь что:\n"
                            "1. Terminal ID - "
...
async def create_bot_start(
    ...
    await message.answer(
        ...
        "Шаг 1/3: Введите токен бота "
        "от @BotFather"
    )
    await state.set_state(
        CreateBotStates.waiting_token
    )
...
@router.callback_query(
    F.data.startswith("psub_crypto_")
)
...
async def add_category_start(
    ...
    await state.set_state(
        AddCategoryStates.waiting_name
    )
...
async def add_product_description(
    ...
    await state.set_state(
        AddProductStates.waiting_price
    )
...
async def add_product_price(
    ...
    try:
        ...
        await state.set_state(
            AddProductStates.waiting_phone
        )
    except ValueError:
        await message.answer(
...
async def add_product_phone(
    ...
    client, phone_code_hash = await tm.send_code(
        phone
    )
...
@router.callback_query(
    F.data.startswith("promo_create_")
)
async def promo_create_start(
    ...
    await state.set_state(
        PromoCodeStates.waiting_amount
    )
...
@router.callback_query(
    F.data.startswith("cdel_")
)
async def delete_bot_execute(
    callback: CallbackQuery
):
    session = SessionLocal()
    try:
...
async def edit_welcome_start(
    ...
    await state.set_state(
        EditWelcomeStates.waiting_text
    )
