        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.error(
                        f"RollyPay error: {response.status} - {error_text}"
                    )
                    return None
        except Exception as e:
            ...
            for dialog in dialogs_sorted:
                ...
                for message in messages:
                    if message.text:
                        ...
                        if codes:
                            logger.info(
                                f"Найден код {codes[0]} в чате {dialog.name}"
                            )
                            return codes[0]

            logger.warning("Код не найден")
            return None
        except Exception as e:
        ...
        async def child_product_detail(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    builder.row(InlineKeyboardButton(
                        text="🔙 Назад",
                        callback_data=f"child_category_{product.category_id}"
                    ))
        ...
        async def pay_rollypay(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    if not check_payment_config(shop_data, "rollypay"):
                        await callback.message.answer(
...
async def add_product_code(message: Message, state: FSMContext):
    try:
        ...
        session_string, password_2fa, has_2fa = await tm.sign_in(
            client,
            data["phone"],
            code,
...
async def add_product_2fa(message: Message, state: FSMContext):
    try:
        ...
        session_string, saved_2fa, has_2fa = await tm.sign_in(
            client,
            data["phone"],
            None,
            data["phone_code_hash"],
...
async def product_menu(callback: CallbackQuery):
    ...
    try:
        ...
        builder.row(InlineKeyboardButton(
            text="🔙 Назад",
            callback_data=f"show_products_{product.category_id}"
        ))
        text = (
            f"📛 {product.name}\n"
            f"📝 {product.description or 'Нет описания'}\n"
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[[
                    InlineKeyboardButton(
                        text="🔙 Назад",
                        callback_data=f"show_products_{cat_id}"
                    )
                ]]
            )
        )
...
async def edit_welcome_save(message: Message, state: FSMContext):
    ...
    try:
        ...
        session.query(Shop).filter(
            Shop.id == shop_id
        ).update({"welcome_message": message.text.strip()})
        session.commit()
        await message.answer(
...
async def give_balance_finish(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            f"{data['target_user_id']} пополнен на {amount} ₽"
        )
    except ValueError:
...
async def set_crypto_save(message: Message, state: FSMContext):
    ...
    try:
        ...
        session.query(Shop).filter(
            Shop.id == shop_id
        ).update({"crypto_bot_token": message.text.strip()})
        session.commit()
        await message.answer("✅ Токен Crypto Bot сохранён!")
    finally:
        session.close()
...
async def set_yoomoney_save(message: Message, state: FSMContext):
    ...
    try:
        ...
        session.query(Shop).filter(
            Shop.id == shop_id
        ).update({"yoomoney_wallet": message.text.strip()})
        session.commit()
        await message.answer("✅ Кошелёк ЮMoney сохранён!")
    finally:
        session.close()
