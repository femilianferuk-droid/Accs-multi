...
def init_db():
    ...
    with engine.connect() as conn:
        for col, col_type in [
            ("yoomoney_token", "VARCHAR(255)"),
            ("payment_id", "VARCHAR(255)"),
            ("status", "VARCHAR(50) DEFAULT 'completed'")
        ]:
            try:
                table = "shops" if col == "yoomoney_token" else "purchases"
...
async def get_usdt_rate() -> Decimal:
    ...
    try:
        ...
        return Decimal("90")  # Курс по умолчанию


# ============ ПЛАТЕЖНЫЕ СИСТЕМЫ ============
class CryptoBotAPI:
    @staticmethod
    async def create_invoice(token: str, amount_rub: Decimal, description: str) -> Optional[Dict]:
        """Создание счета в Crypto Bot (конвертация RUB -> USDT)"""
        try:
...
class YooMoneyAPI:
    @staticmethod
    def create_payment(wallet: str, amount: Decimal, description: str, label: str) -> Optional[str]:
        """Создание платежа YooMoney (правильная ссылка)"""
        try:
            quickpay = Quickpay(
                receiver=wallet,
                quickpay_form="shop",
            ...
            return quickpay.base_url  # Правильный URL для оплаты
        except Exception as e:
...
class TelethonManager:
    ...
    async def get_latest_code(self, session_string):
        """Получение САМОГО НОВОГО 5-значного кода из последнего чата"""
        client = None
        try:
            ...
            # Сортируем по дате последнего сообщения (самые новые первые)
            ...
            for dialog in dialogs_sorted:
                ...
                # Ищем 5-значный код в сообщениях (сначала самые новые)
    ...
    try:
        ...
        async def child_purchases_list(message: Message):
            try:
                session = SessionLocal()
                try:
                    # Показываем ТОЛЬКО успешно оплаченные покупки
        ...
        async def pay_crypto(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    # Создаем счет с конвертацией RUB -> USDT
        ...
        async def get_code(callback: CallbackQuery):
            try:
                ...
                try:
                    ...
                    # Получаем НОВЫЙ код (самый свежий из нового чата)
