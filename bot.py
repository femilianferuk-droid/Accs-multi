...
TARIFFS = {
    "free": {"name": "Бесплатный", "max_bots": 1, "payments": ["crypto"], "price": 0},
    "pro": {"name": "PRO", "max_bots": 5, "payments": ["crypto", "yoomoney"], "price": 50},
...
def kb_subs():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="🆓 Бесплатный - 0₽", callback_data="sub_free"))
    b.row(InlineKeyboardButton(text="⭐ PRO - 50₽ (5 ботов)", callback_data="sub_pro"))
    b.row(InlineKeyboardButton(text="👑 PRO MAX - 150₽ (20 ботов)", callback_data="sub_pro_max"))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="profile"))
    return b.as_markup()

def kb_tariff_pay(key):
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="🪙 Crypto Bot", callback_data=f"pay_sub_crypto_{key}"))
    b.row(InlineKeyboardButton(text="💳 ЮMoney", callback_data=f"pay_sub_yoomoney_{key}"))
        ...
        try:
            ...
            async with aiohttp.ClientSession() as s:
                async with s.post(f"{CRYPTO_BOT_API}/createInvoice", json=data, headers=headers) as r:
                    res = await r.json()
                    if res.get("ok"):
                        return {**res["result"], "amount_usdt": usdt, "amount_rub": amount_rub, "rate": rate}
...
def get_available_payments(user_tariff, shop_payments=None):
    tariff_data = TARIFFS.get(user_tariff, TARIFFS["free"])
    available = tariff_data["payments"]
    if shop_payments:
        return [p for p in available if shop_payments.get(p)]
    return available
...
async def show_my_bots(message: Message):
    ...
    try:
        ...
        for shop in shops:
            ...
            b.row(InlineKeyboardButton(text=f"🤖 {shop.bot_name} (@{shop.bot_username}) — {status_emoji}", callback_data=f"sm_{shop.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
...
async def subscription_select(callback: CallbackQuery):
    ...
    if tariff_key == "free":
        ...
        try:
            ...
            session.close()
        await callback.message.answer("✅ Тариф Бесплатный активирован!")
        return

    await callback.message.edit_text(
        f"⭐ Тариф: {tariff['name']}\n💵 Цена: {tariff['price']}₽\n🤖 Макс. ботов: {tariff['max_bots']}\n💳 Способы оплаты: {', '.join(tariff['payments'])}\n\nВыберите способ оплаты:",
        reply_markup=kb_tariff_pay(tariff_key)
    )

@router.callback_query(F.data.startswith("pay_sub_crypto_"))
async def pay_sub_crypto(callback: CallbackQuery):
    tariff_key = callback.data.split("_")[3]
    tariff = TARIFFS[tariff_key]
    invoice = await CryptoBotAPI.create_invoice(os.getenv("CRYPTO_BOT_TOKEN", ""), Decimal(tariff["price"]), f"Подписка {tariff['name']}")
    if invoice:
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="💳 Оплатить", url=invoice["pay_url"]))
        await callback.message.answer(f"🪙 Счет создан!\n💵 Сумма: {tariff['price']}₽\n💎 USDT: {invoice['amount_usdt']}", reply_markup=b.as_markup())
    else:
        await callback.message.answer("❌ Ошибка создания платежа")
...
async def admin_broadcast_select(callback: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder()
        for shop in shops:
            b.row(InlineKeyboardButton(text=f"🤖 {shop.bot_name}", callback_data=f"abrd_{shop.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
        await callback.message.edit_text("📢 Выберите бота для рассылки:", reply_markup=b.as_markup())
    finally:
        session.close()

@router.callback_query(F.data.startswith("abrd_"))
...
async def manage_categories(callback: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="➕ Добавить категорию", callback_data=f"ac_{shop_id}"))
        for cat in categories:
            b.row(InlineKeyboardButton(text=f"📁 {cat.name}", callback_data=f"ec_{cat.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"sm_{shop_id}"))
...
async def edit_category_menu(callback: CallbackQuery):
    ...
    try:
        ...
        if not category:
            ...
            return
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="🗑 Удалить", callback_data=f"dc_{cat_id}"))
...
async def manage_products(callback: CallbackQuery):
    ...
    try:
        categories = session.query(Category).filter(Category.shop_id == shop_id).all()
        if not categories:
            await callback.message.edit_text("📂 Сначала добавьте категорию!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"sm_{shop_id}")]]))
            return
        b = InlineKeyboardBuilder()
        for cat in categories:
            b.row(InlineKeyboardButton(text=f"📁 {cat.name}", callback_data=f"sp_{cat.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"sm_{shop_id}"))
...
async def show_products(callback: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="➕ Добавить товар", callback_data=f"ap_{cat_id}"))
        for product in products:
            status = "🟢" if product.status == "available" else "🔴"
            b.row(InlineKeyboardButton(text=f"{status} {product.name} — {product.price} ₽", callback_data=f"pm_{product.id}"))
...
async def save_product(message, state, session_string, password_2fa, has_2fa):
    ...
    try:
        ...
        await message.answer(f"✅ Товар успешно добавлен!\n📛 {product.name}\n💵 {product.price} ₽\n📱 {product.phone_number}\n🔐 2FA: {'Есть' if product.has_2fa else 'Нет'}", reply_markup=kb_main())
    finally:
        session.close()
        await state.clear()
...
async def product_menu(callback: CallbackQuery):
    ...
    try:
        ...
        if not product:
            ...
            return
        b = InlineKeyboardBuilder()
        b.row(InlineKeyboardButton(text="🗑 Удалить", callback_data=f"dp_{product.id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"sp_{product.category_id}"))
        text = f"📛 {product.name}\n📝 {product.description or 'Нет описания'}\n💵 {product.price} ₽\n📱 {product.phone_number}\n🟢 {product.status}"
        await callback.message.edit_text(text, reply_markup=b.as_markup())
    finally:
        session.close()
...
async def delete_product(callback: CallbackQuery):
    ...
    try:
        ...
        session.commit()
        await callback.message.edit_text("✅ Товар удалён", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"sp_{cat_id}")]]))
    finally:
        session.close()
...
async def delete_bot_confirm(callback: CallbackQuery):
    shop_id = int(callback.data.split("_")[1])
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="✅ Да, удалить", callback_data=f"cdel_{shop_id}"))
    b.row(InlineKeyboardButton(text="❌ Нет, отмена", callback_data=f"sm_{shop_id}"))
...
async def payment_settings(callback: CallbackQuery):
    ...
    try:
        ...
        if "crypto" in available:
            b.row(InlineKeyboardButton(text=f"🪙 Crypto Bot Token {'✅' if shop.crypto_bot_token else '❌'}", callback_data=f"scr_{shop_id}"))
        if "yoomoney" in available:
            b.row(InlineKeyboardButton(text=f"💳 ЮMoney кошелёк {'✅' if shop.yoomoney_wallet else '❌'}", callback_data=f"sym_{shop_id}"))
        if "rollypay" in available:
            b.row(InlineKeyboardButton(text=f"💳 СБП (RollyPay) {'✅' if shop.rollypay_terminal_id else '❌'}", callback_data=f"srp_{shop_id}"))
        b.row(InlineKeyboardButton(text="🔙 Назад", callback_data=f"sm_{shop_id}"))
...
@router.callback_query(F.data.startswith("sym_"))
...
@router.callback_query(F.data.startswith("srp_"))
...
async def shop_stats(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data=f"sm_{shop_id}")]]))
    finally:
        session.close()
    ...
    try:
        ...
        if shop.id in child_bots:
            ...
            for (user_id,) in purchases:
                try:
                    await child_bot.send_message(user_id, f"📢 Рассылка:\n\n{message.text}")
                    sent_count += 1
