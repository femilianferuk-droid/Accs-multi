...
Aiogram 3.x + PostgreSQL (asyncpg) + Telethon
"""
...
from sqlalchemy import (
    ...
    Boolean, DateTime, ForeignKey, select, update, and_
)
...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql+asyncpg://postgres:password@localhost:5432/vest_multi")
...
# Важно: правильное создание асинхронного движка
...
async def get_or_create_user(telegram_id: int, username: str, session: AsyncSession) -> User:
    ...
    try:
        ...
        async def child_start(message: Message):
            try:
                async with async_session() as s:
                    shop_data = await s.execute(select(Shop).where(Shop.id == shop.id))
                    shop_data = shop_data.scalar_one_or_none()

                    if not shop_data:
                        await message.answer("❌ Магазин не найден")
                        return

                    user = await get_or_create_user(message.from_user.id, message.from_user.username or "", s)
            ...
            try:
                async with async_session() as s:
                    categories = await s.execute(
                        select(Category).where(Category.shop_id == shop.id)
                    )
                ...
                async with async_session() as s:
                    products = await s.execute(
                        select(Product).where(
                            and_(Product.category_id == cat_id, Product.status == "available")
                        )
                    )
        ...
        async def child_product_detail(callback: CallbackQuery):
            try:
                ...
                async with async_session() as s:
                    product = await s.execute(select(Product).where(Product.id == product_id))
...
"""
                    builder = InlineKeyboardBuilder()
                    builder.row(InlineKeyboardButton(
                        text="🪙 Crypto Bot", callback_data=f"pay_crypto_{product.id}"
                    ))
                    builder.row(InlineKeyboardButton(
                        text="💳 ЮMoney", callback_data=f"pay_yoomoney_{product.id}"
                    ))
                    builder.row(InlineKeyboardButton(
                        text="💰 Баланс", callback_data=f"pay_balance_{product.id}"
                    ))
                    builder.row(InlineKeyboardButton(
                        text="🔙 Назад", callback_data=f"child_cat_{product.category_id}"
                    ))
        ...
        async def pay_balance(callback: CallbackQuery):
            try:
                product_id = int(callback.data.split("_")[2])
                async with async_session() as s:
                    user = await get_user(callback.from_user.id, s)
                    product = await s.execute(select(Product).where(Product.id == product_id))
                    product = product.scalar_one_or_none()
...
""",
        ...
        async def get_code(callback: CallbackQuery):
            try:
                ...
                async with async_session() as s:
                    purchase = await s.execute(select(Purchase).where(Purchase.id == purchase_id))
                    ...
                    product = await s.execute(select(Product).where(Product.id == purchase.product_id))
                    product = product.scalar_one_or_none()
        ...
        async def child_profile(callback: CallbackQuery):
            try:
                async with async_session() as s:
                    user = await get_user(callback.from_user.id, s)
                    purchases = await s.execute(
                        select(Purchase).where(Purchase.user_id == callback.from_user.id)
                        .order_by(Purchase.created_at.desc()).limit(10)
                    )
...
"""
        ...
        async def child_support_handler(callback: CallbackQuery):
            try:
                ...
                logger.error(f"❌ Ошибка child_support: {e}")
        ...
        async def child_back(callback: CallbackQuery):
            try:
                ...
                logger.error(f"❌ Ошибка child_back: {e}")
...
async def cmd_start(message: Message):
    try:
        async with async_session() as session:
            await get_or_create_user(message.from_user.id, message.from_user.username or "", session)
...
"""
        ...
        logger.error(f"❌ Ошибка show_profile: {e}")
    ...
    try:
        async with async_session() as session:
            shops = await session.execute(
                select(Shop).where(Shop.owner_id == callback.from_user.id)
            )
        ...
        logger.error(f"❌ Ошибка show_my_bots: {e}")
...
async def shop_menu(callback: CallbackQuery):
    try:
        ...
        async with async_session() as session:
            shop = await session.execute(select(Shop).where(Shop.id == shop_id))
...
"""
        ...
        logger.error(f"❌ Ошибка shop_menu: {e}")
...
async def manage_categories(callback: CallbackQuery):
    try:
        ...
        logger.error(f"❌ Ошибка manage_categories: {e}")
...
async def add_category_finish(message: Message, state: FSMContext):
    try:
        ...
        logger.error(f"❌ Ошибка add_category: {e}")
    finally:
        await state.clear()
...
async def manage_products(callback: CallbackQuery):
    try:
        ...
        logger.error(f"❌ Ошибка manage_products: {e}")
...
async def show_products(callback: CallbackQuery):
    try:
        cat_id = int(callback.data.split("_")[2])
        async with async_session() as session:
            category = await session.execute(select(Category).where(Category.id == cat_id))
        ...
        logger.error(f"❌ Ошибка show_products: {e}")
...
async def save_product(message: Message, state: FSMContext, session_string: str,
    ...
    try:
        data = await state.get_data()
        async with async_session() as session:
            category = await session.execute(select(Category).where(Category.id == data["category_id"]))
            category = category.scalar_one_or_none()
...
async def product_menu(callback: CallbackQuery):
    try:
        ...
        async with async_session() as session:
            ...
            builder = InlineKeyboardBuilder()
            builder.row(InlineKeyboardButton(
                text="🗑 Удалить", callback_data=f"delete_product_{product.id}"
            ))
            builder.row(InlineKeyboardButton(
                text="🔙 Назад", callback_data=f"show_products_{product.category_id}"
            ))
...
"""
        ...
        logger.error(f"❌ Ошибка product_menu: {e}")
    ...
    try:
        ...
        async with async_session() as session:
            await session.execute(
                update(Shop).where(Shop.id == shop_id).values(welcome_message=message.text.strip())
            )
            await session.commit()
        await message.answer("✅ Приветственное сообщение обновлено!")
    except Exception as e:
        logger.error(f"❌ Ошибка edit_welcome: {e}")
    finally:
        await state.clear()
...
async def edit_support_save(message: Message, state: FSMContext):
    try:
        ...
        async with async_session() as session:
            await session.execute(
                update(Shop).where(Shop.id == shop_id).values(support_username=username)
            )
            await session.commit()
        await message.answer(f"✅ Юзернейм поддержки обновлён: @{username}")
    except Exception as e:
        logger.error(f"❌ Ошибка edit_support: {e}")
    finally:
        await state.clear()
...
async def give_balance_finish(message: Message, state: FSMContext):
    try:
        ...
        data = await state.get_data()
        async with async_session() as session:
            user = await get_user(data["target_user_id"], session)
            if not user:
        ...
        logger.error(f"❌ Ошибка give_balance: {e}")
    finally:
        await state.clear()
...
async def payment_settings(callback: CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        text=f"🪙 Crypto Bot Token ({crypto_status})",
        callback_data=f"set_crypto_{shop_id}"
    ))
    builder.row(InlineKeyboardButton(
        text=f"💳 ЮMoney кошелёк ({yoomoney_status})",
...
async def set_crypto_save(message: Message, state: FSMContext):
    try:
        ...
        async with async_session() as session:
            await session.execute(
                update(Shop).where(Shop.id == shop_id).values(crypto_bot_token=message.text.strip())
            )
            await session.commit()
        await message.answer("✅ Токен Crypto Bot сохранён!")
    except Exception as e:
        logger.error(f"❌ Ошибка set_crypto: {e}")
    finally:
        await state.clear()
...
async def set_yoomoney_save(message: Message, state: FSMContext):
    try:
        ...
        async with async_session() as session:
            await session.execute(
                update(Shop).where(Shop.id == shop_id).values(yoomoney_wallet=message.text.strip())
            )
            await session.commit()
        await message.answer("✅ Кошелёк ЮMoney сохранён!")
    except Exception as e:
        logger.error(f"❌ Ошибка set_yoomoney: {e}")
    finally:
        await state.clear()
...
async def on_startup():
    ...
    async with async_session() as session:
        ...
        result = await session.execute(select(Shop).where(Shop.status == "active"))
